...
export default function AdminLayout() {
  ...
  const active = menuItems.find((m) => loc.pathname.startsWith(m.key))?.key ?? '/dashboard';

  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Sider width={220} theme="dark" style={{ borderRight: '1px solid rgba(0,212,255,0.1)' }}>
        <div style={{ padding: '20px 24px', ...brandStyle }}>
          IMAGE · RELAY
        </div>
        ...
        <Header style={headerStyle}>
          ...
          >
            <Space style={{ cursor: 'pointer', color: '#E6ECF5' }}>
              <Avatar size="small" style={{ background: '#00D4FF', color: '#0A0F1D' }}>
